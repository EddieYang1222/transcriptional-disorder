# Allele Specific Expression Analysis (Medina-Cano)

# Online links
# https://pubmed.ncbi.nlm.nih.gov/40449493/
# https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE268333
...
######################################################
# Load in data
# Neocortical organoid snRNA-seq of B6 crossed with four strains (CAST, PWK,
# SPRET, MOLF). Each cross is analyzed separately, with allelic discordance
# centered within cross.
data_dir <- 'path/to/GSE268333_allele_matrices'
cell_labels <- read.delim(file.path(data_dir, "organoid_cell_labels.tsv"), stringsAsFactors = FALSE)
...
# Crosses with fewer than 50 cells are skipped.
MIN.READS = 5
FRAC.CELLS = 0.20
...
for (cr in crosses) {
  ...
  cells <- intersect(cell_labels$barcode[cell_labels$cross == cr], barcodes)
  if (length(cells) < 50) next
  Aallele <- Aallele[, cells, drop = FALSE]
  Ballele <- Ballele[, cells, drop = FALSE]

  # Gene filter: covered in at least 20% of the cross's cells
  ...
  # save(ABallele.saver, file = paste0("ABallele_MedinaCano_", cr, "_SAVER.RData"))
...
write.csv(data.frame(cross = names(partial_rho_by_cross), partial_rho = partial_rho_by_cross),
          "dyscoordination_vs_discordance_MedinaCano_by_cross.csv", row.names = FALSE)
