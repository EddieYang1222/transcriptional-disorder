# Allele Specific Expression Analysis (Larsson)

# Online links
# https://pubmed.ncbi.nlm.nih.gov/30602787/
# https://github.com/sandberg-lab/txburst
# https://www.ebi.ac.uk/biostudies/arrayexpress/studies/E-MTAB-6362
# https://www.ebi.ac.uk/biostudies/arrayexpress/studies/E-MTAB-6385
# https://www.ebi.ac.uk/biostudies/arrayexpress/studies/E-MTAB-7098

# Set up working directory
# setwd("S:/Penn Dropbox/Eddie Yang/Aging/Scripts/Code/allele_specific_analysis/larsson")

######################################################
# Load in data
counts_c57 <- read.csv("SS3_c57_UMIs_mESC.csv", header = TRUE, row.names = 1, sep = ",")
counts_c57 <- as.matrix(counts_c57)
counts_c57[is.na(counts_c57)] <- 0

counts_cast <- read.csv("SS3_cast_UMIs_mESC.csv", header = TRUE, row.names = 1, sep = ",")
counts_cast <- as.matrix(counts_cast)
counts_cast[is.na(counts_cast)] <- 0

# Remove genes with zero counts
index_genes <- rowSums(counts_c57 + counts_cast) != 0
counts_c57 <- counts_c57[index_genes,]
...
nrow(Aallele)/nrow(counts_c57) # 0.04789816
...
# save(ABallele.saver, file = "ABallele_Larsson_SAVER.RData")
# load("ABallele_Larsson_SAVER.RData")
