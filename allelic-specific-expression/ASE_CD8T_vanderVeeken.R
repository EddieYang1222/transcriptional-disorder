# Allele Specific Expression Analysis (van der Veeken)

# Online links
# https://pubmed.ncbi.nlm.nih.gov/31201093/
# https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE126770
...
# genes, with the CD8 T cell type annotation.
data_dir <- 'path/to/GSE126770_allele_matrices'
Aallele <- as.matrix(Matrix::readMM(file.path(data_dir, "Aallele_B6.mtx")))
Ballele <- as.matrix(Matrix::readMM(file.path(data_dir, "Ballele_SPRET_EiJ.mtx")))
...
cell_labels <- read.delim(file.path(data_dir, "tcell_cell_labels.tsv"), stringsAsFactors = FALSE)
...
# Progenitor-exhausted cells are excluded (too few cells).
MIN.READS = 5
FRAC.CELLS = 0.20

all_temp_gene_level <- data.frame()

for (ct in c("Effector/Cytotoxic", "Proliferating")) {
  ...
  # save(ABallele.saver, file = paste0("ABallele_vanderVeeken_", gsub("[/ ]", "-", ct), "_SAVER.RData"))
...
p <- ggplot(data.frame(x = res_dev, y = res_disc), aes(x = x, y = y)) +
  ...
  ggtitle(sprintf("van der Veeken: partial rho = %.2f", partial_rho)) +
  theme(panel.grid = element_blank(), panel.background = element_blank(),
        plot.background = element_blank(), axis.line = element_line(color = "black"))

ggsave("dyscoordination_vs_discordance_vanderVeeken.pdf", plot = p)
